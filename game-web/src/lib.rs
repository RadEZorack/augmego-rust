...
impl WebApp {
        ...
        if let Some(permission) = permission_update {
            ...
            match permission {
                MobileTiltPermissionState::Active => {
                    if let Some(sample) = self.mobile_controls.latest_tilt_sample {
    ...
    fn attach_mobile_tilt_listener_if_needed(&mut self) {
        ...
        let on_orientation = Closure::wrap(Box::new(move |event: WebEvent| {
            ...
            let alpha = event.alpha().unwrap_or(0.0) as f32;
    ...
    fn update_weapon_collection_panel(&self) {
        ...
        let _ = self.weapons_panel.set_attribute("style", "display:none;");
    }
...
fn create_mobile_hud() -> MobileHud {
    ...
    let control_hint = document
        ...
        .unwrap_or_else(fallback_element);
...
fn default_mobile_tilt_baseline(sample: MobileTiltSample) -> MobileTiltSample {
    let (beta_deg, gamma_deg) = upright_mobile_tilt_pose(sample.orientation);
    MobileTiltSample {
...
fn upright_mobile_tilt_pose(orientation: MobileViewportOrientation) -> (f32, f32) {
    match orientation {
        MobileViewportOrientation::Portrait => (90.0, 0.0),
        MobileViewportOrientation::PortraitUpsideDown => (-90.0, 0.0),
        MobileViewportOrientation::LandscapeLeft => (0.0, -90.0),
        MobileViewportOrientation::LandscapeRight => (0.0, 90.0),
    }
}

fn mobile_tilt_quaternion(sample: MobileTiltSample) -> Quat {
    let alpha = sample.alpha_deg.to_radians();
    let beta = sample.beta_deg.to_radians();
    let gamma = sample.gamma_deg.to_radians();
    let screen = viewport_orientation_radians(sample.orientation);
    Quat::from_euler(EulerRot::YXZ, alpha, beta, -gamma)
        * Quat::from_rotation_x(-std::f32::consts::FRAC_PI_2)
        * Quat::from_rotation_z(-screen)
}

fn viewport_orientation_radians(orientation: MobileViewportOrientation) -> f32 {
    match orientation {
        MobileViewportOrientation::Portrait => 0.0,
        MobileViewportOrientation::LandscapeLeft => std::f32::consts::FRAC_PI_2,
...
fn avatar_generation_square_media_style(display: &str) -> String {
    format!(
        "display:{display};width:min(100%,320px);aspect-ratio:1 / 1;height:auto;object-fit:cover;border-radius:18px;border:1px solid rgba(19,33,42,0.14);background:rgba(5,8,12,0.45);margin:0 auto;"
    )
}

fn avatar_generation_portrait_media_style(display: &str) -> String {
    format!(
        "display:{display};width:100%;max-height:320px;object-fit:contain;border-radius:18px;border:1px solid rgba(19,33,42,0.14);background:rgba(5,8,12,0.45);"
    )
}

fn set_element_image_src(image: &Element, url: Option<&str>, visible_style: &str) {
    match url {
        Some(url) if !url.trim().is_empty() => {
            let _ = image.set_attribute("src", url);
            let _ = image.set_attribute("style", visible_style);
        }
        _ => {
...
fn render_avatar_generation_modal(
    ...
    set_element_image_src(
        portrait_preview,
        portrait_url,
        &avatar_generation_portrait_media_style("block"),
    );
...
fn create_player_avatar_panel() -> (Element, Element, Element, Closure<dyn FnMut(WebEvent)>) {
    ...
    let _ = live_video.set_attribute("style", &avatar_generation_square_media_style("block"));
    ...
    upload_wrapper.set_text_content(Some("Upload Selfie Fallback (Optional)"));
    ...
    let retake_onclick = Closure::wrap(Box::new(move |_event: WebEvent| {
        ...
        let state = generation_state_for_retake.clone();
        let selfie_preview = selfie_preview_for_retake.clone();
        let portrait_preview = portrait_preview_for_retake.clone();
        let progress_fill = progress_fill_for_retake.clone();
        let progress_label = progress_label_for_retake.clone();
        let step_list = step_list_for_retake.clone();
...
mod tests {
    ...
    #[test]
    fn upright_mobile_tilt_pose_respects_orientation() {
        assert_eq!(
            upright_mobile_tilt_pose(MobileViewportOrientation::Portrait),
            (90.0, 0.0)
        );
        assert_eq!(
            upright_mobile_tilt_pose(MobileViewportOrientation::LandscapeLeft),
            (0.0, -90.0)
        );
    }
    ...
    #[test]
    fn chair_spin_changes_tilt_yaw() {
        let baseline = MobileTiltSample {
            alpha_deg: 15.0,
            beta_deg: 90.0,
            gamma_deg: 0.0,
    ...
    fn upright_portrait_pose_is_the_default_tilt_neutral() {
        let sample = MobileTiltSample {
            alpha_deg: 32.0,
            beta_deg: 90.0,
            gamma_deg: 0.0,
            orientation: MobileViewportOrientation::Portrait,
        };
        let baseline = default_mobile_tilt_baseline(sample);
        let offset = mobile_tilt_offset_from_samples(sample, baseline);
        assert!(offset.length() < 1.0e-3);
    }
