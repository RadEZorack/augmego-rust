...
use wgpu_lite::{Mesh, Renderer, TexturedMesh, Vertex};
...
const FEATURED_MODEL_PATH: &str = concat!(
...
fn main() -> Result<()> {
    ...
    let mut featured_model: Option<TexturedMesh> = None;
    let mut featured_model_attempted = false;
        ...
        match event {
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::RedrawRequested => {
                    ...
                    if !featured_model_attempted {
                        if let Some(spawn_position) = app.spawn_position() {
                            featured_model_attempted = true;
                            match load_featured_model_mesh(&renderer, spawn_position) {
                                Ok(mesh) => featured_model = Some(mesh),
                                Err(error) => eprintln!("failed to load featured model: {error:?}"),
                            }
                        }
                    }
                    ...
                    let mut textured_mesh_refs = Vec::new();
                    if let Some(mesh) = &featured_model {
                        textured_mesh_refs.push(mesh);
                    }
...
impl GameApp {
        ...
        while let Ok(event) = self.network_rx.try_recv() {
            match event {
                ...
                NetworkEvent::Welcome { message, spawn_position } => {
                    ...
                    self.spawn_position = Some(spawn_position);
                }
    ...
    fn spawn_position(&self) -> Option<WorldPos> {
        self.spawn_position
    }
...
fn load_featured_model_mesh(renderer: &Renderer<'_>, spawn_position: WorldPos) -> Result<TexturedMesh> {
    let (mut vertices, indices) = load_glb_vertices(Path::new(FEATURED_MODEL_PATH))?;
    let (min, max) = model_bounds(&vertices).context("featured model has no vertices")?;
    let size = max - min;
    let model_height = size.y.max(0.001);
    let scale = FEATURED_MODEL_DESIRED_HEIGHT / model_height;
    ...
    let final_vertices = vertices
        .drain(..)
        .map(|vertex| {
            let normalized = Vec3::new(vertex.position.x - center_x, vertex.position.y - min.y, vertex.position.z - center_z);
            Vertex {
    ...
    let texture = renderer.create_dynamic_texture(1, 1);
    renderer.update_dynamic_texture_rgba(&texture, &[255, 255, 255, 255]);
    Ok(renderer.create_textured_mesh(&final_vertices, &indices, &texture))
}

fn load_glb_vertices(path: &Path) -> Result<(Vec<ImportedVertex>, Vec<u32>)> {
    let (document, buffers, _) = gltf::import(path).with_context(|| format!("import {}", path.display()))?;
    let scene = document
        .default_scene()
        .or_else(|| document.scenes().next())
        .context("glb has no scene")?;
    let mut vertices = Vec::new();
    let mut indices = Vec::new();

    for node in scene.nodes() {
        append_gltf_node_meshes(&node, Mat4::IDENTITY, &buffers, &mut vertices, &mut indices);
    }
...
fn append_gltf_node_meshes(
    ...
    for child in node.children() {
        append_gltf_node_meshes(&child, transform, buffers, vertices, indices);
    }
}
